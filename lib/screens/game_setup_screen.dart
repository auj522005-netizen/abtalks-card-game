...
class _GameSetupScreenState extends State<GameSetupScreen> {
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Padding(padding: const EdgeInsets.all(24), child: Column(children: [
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), decoration: BoxDecoration(color: _players.length >= 2 ? const Color(0xFFE94560).withOpacity(0.15) : const Color(0xFFE94560).withOpacity(0.15), borderRadius: BorderRadius.circular(20)),
            child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(_players.length >= 2 ? Icons.check_circle_rounded : Icons.info_outline_rounded, color: const Color(0xFFE94560), size: 16), const SizedBox(width: 6), Text('${_players.length} لاعب', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 14))])),
          const SizedBox(height: 16),
  ...
  Widget _playerTile(int i) {
    return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
      ...
      child: Row(children: [
        Container(width: 40, height: 40, decoration: BoxDecoration(color: const Color(0xFFE94560).withOpacity(0.2), shape: BoxShape.circle), child: Center(child: Text(_players[i].isNotEmpty ? _players[i][0].toUpperCase() : '?', style: const TextStyle(color: Color(0xFFE94560), fontWeight: FontWeight.bold, fontSize: 18)))),
